...
void wall_collision_verlet(Common *p,int N,double dt,double mu,TriangleMesh* mesh){
    for(int i=0; i<N; i++){
...
void particle_collision_verlet(Common* p,int N,double dt,double mu, BoundingBox *box){
    for (int i=0; i<N; i++){
        if(p->isActive[i]!=1){
            continue;
        }
        ...
        for (int k=0; k<end; k++){
            ...
            if (distsq<R*R){
                ...
                calc_tangential_force(p,i,j,dt,mu,c);
...
int shouldRefreshNeighborList(Common *p,int N, BoundingBox* box){
    int flag = 0;
    double threshSq = box->refreshThreshSq;
    for(int i=0; i<N; i++){
        if(p->isActive[i]!=1){
...
void cpu_dem_verlet_verlet(ParticleSys<HostMemory>* p, ParticleSys<HostMemory>* tmpP, BoundingBox* box,TriangleMesh *mesh,BVH *bvh, int step){
    ...
    for (int i=0; i<N*MAX_NEI; i++){
        p->p.isContact[i]=0;
        p->p.isContactWall[i]=0;
    }



    particle_collision_verlet(&p->p,N,dt,mu,box);

    wall_collision_verlet(&p->p,N,dt,mu,mesh);
    ...
    for (int i = 0; i < N; i++){
        ...
        // acceleration
        p->p.a[bi+0] = p->p.f[bi+0]*p->p.invm[i]+p->p.g[0];
        p->p.a[bi+1] = p->p.f[bi+1]*p->p.invm[i]+p->p.g[1];
        p->p.a[bi+2] = p->p.f[bi+2]*p->p.invm[i]+p->p.g[2];

        // velocity
        p->p.v[bi+0] += p->p.a[bi+0] * dt;
        p->p.v[bi+1] += p->p.a[bi+1] * dt;
        p->p.v[bi+2] += p->p.a[bi+2] * dt;

        // position
        p->p.x[bi+0] += p->p.v[bi+0] * dt;
        p->p.x[bi+1] += p->p.v[bi+1] * dt;
        p->p.x[bi+2] += p->p.v[bi+2] * dt;

        // angular acceleration
        p->p.anga[bi+0] = p->p.mom[bi+0]*p->p.invmoi[i];
        p->p.anga[bi+1] = p->p.mom[bi+1]*p->p.invmoi[i];
        p->p.anga[bi+2] = p->p.mom[bi+2]*p->p.invmoi[i];

        // angular velocity
        p->p.angv[bi+0] += p->p.anga[bi+0] * dt;
        p->p.angv[bi+1] += p->p.anga[bi+1] * dt;
        p->p.angv[bi+2] += p->p.anga[bi+2] * dt;
    }

    int flag = shouldRefreshNeighborList(&p->p,N,box);
    if (flag ==1){
        update_neighborlist(p,tmpP,box);
        update_neighborlist_wall(&p->p,N,mesh,bvh,box->skinR);
